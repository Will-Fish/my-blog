...
type HomePhotographyPreviewProps = {
  images: PhotographyImage[];
};

export function HomePhotographyPreview({ images }: HomePhotographyPreviewProps) {
  ...
  return (
    <>
      <div className="grid auto-rows-[88px] grid-cols-6 gap-2 sm:auto-rows-[108px] sm:gap-3 lg:hidden">
        {images.map((item, index) => {
          ...
          return (
            <Card
              key={item.id}
              className={`group overflow-hidden rounded-[12px] ${tileClass}`}
            >
              <button
                type="button"
